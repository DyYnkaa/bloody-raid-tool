...
from niggerscraper import scraper; from niggerscraper import selfscraper; from auth import *
import emoji as ej
...
r = Fore.RESET; c = Fore.RED; g = Fore.LIGHTBLACK_EX; tokens = open("data/tokens.txt", "r", encoding="utf8").read().splitlines(); messages = open("data/messages.txt", "r", encoding="utf8").read().splitlines()
...
class Raider:
    ...
    def typing(self, t, message, channelid):
        headers = Headers(t)
        try:
            rr = requests.post(f"https://discord.com/api/v9/channels/{channel_id}/typing", headers=headers); token = t.split(".")[0]
            if rr.status_code == 204: print(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[SUCCESS]   {g}->   {r}Successfully typing {c}{token}{g}****{r}")
            elif rr.status_code == 403: print(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[ERROR]     {g}->    {r}Failed {c}{token}{g}****{r}  {g}{rr.text}{r}")
        except: pass
    ...
    def call(self, t, user_id):
        if len(tokens) == 0:
            ...
            try:
                ...
                rr = request.delete(f'https://discord.com/api/v10/channels/{user_id}/call', headers=headers, json={}); token = t.split(".")[0]
                if rr.status_code == 200: print(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[SUCCESS]   {g}->   {r}Successfully Called {c}{token}{g}****{r}")
                else: print(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[ERROR]     {g}->    {r}Failed {c}{token}{g}****{r}  {g}{rr.text}{r}")
            except: pass

    def silent(self, t, channel_id):
        if len(tokens) == 0:
            print(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[ERROR]     {g}->   {r}Put {c}tokens{r} to {c}data/tokens.txt{r}")
        else:
            headers = Headers(t)
            try:
                rr = request.delete(f'https://discord.com/api/v9/channels/{channel_id}/messages/bulk-delete', headers=headers, json={}); token = t.split(".")[0]
                if rr.status_code == 200: print(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[SUCCESS]   {g}->   {r}Successfully Deleted {c}{token}{g}****{r}")
                else: print(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[ERROR]     {g}->    {r}Failed {c}{token}{g}****{r}  {g}{rr.text}{r}")
            except: pass
...
def main():
    ...
    print(f"""
            {c}«{r}01{c}»{r}  Joiner         {c}«{r}08{c}»{r}  Reacti on Bomber       {c}«{r}15{c}»{r}  Name Changer        {c}«{r}22{c}»{r}  Button Clicker 
            ...
            {c}«{r}05{c}»{r}  Inviter        {c}«{r}12{c}»{r}  Hypesquad Joiner       {c}«{r}19{c}»{r}  Boost Server        {c}«{r}26{c}»{r}  Token Captcher
            {c}«{r}06{c}»{r}  Onliner        {c}«{r}13{c}»{r}  Avatar Changer         {c}«{r}20{c}»{r}  DM Spammer          {c}«{r}27{c}»{r}  Call Spammer
            {c}«{r}07{c}»{r}  Typing         {c}«{r}14{c}»{r}  BIO Changer            {c}«{r}21{c}»{r}  Mass DM             {c}«{r}>>{c}»{r}  Next Page
    """)
    tokenamount = len(open('data/tokens.txt', 'r').readlines())
    idamount = len(open('data/ids.txt', 'r').readlines())
    ...
    if nigger == "": main()
        ...
        os.system('cls'); print(banner)
        channel_id = str(input(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {r}Channel ID {g}» {c}"))
        if channel_id == "": main()
        delay = float(input(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {r}Delay {g}» {c}"))
        os.system('cls'); print(banner)
        raider = Raider()
        for token in tokens:
            threading.Thread(target=raider.typing, args=(token, channel_id)).start()
            time.sleep(delay)
        exit = input(""); exit = main();
        ...
        raider = Raider()
        for token in tokens:
            threading.Thread(target=raider.singledm, args=(token, userid, message)).start()
            time.sleep(delay)
        exit = input(""); exit = main();
